...
public:
    ...
    //! Evaluate the tree using the given mapping of variables to numbers
     virtual void convertIR(std::string dstreg, Context myContext, std::ostream &outStream) const {
        ...
        outStream.push_back(IntermediateRep("BEQ", compare_reg, "reg_0", my_label)); // branch until after the if statement if condition evals to 0
        branch->convertIR(dstreg, myContext, outStream);
        outStream.push_back(IntermediateRep(my_label, "N_A", "N_A", "N_A"));

        outStream.push_back(IntermediateRep("EXITLOCALSCOPE", "N_A", "N_A", "N_A")); // scope needs thinking
        myContext.exitScope();
    }
};
...
public:
    ...
    virtual void convertIR(std::string dstreg, Context myContext, std::ostream &outStream) const {
        ...
        outStream.push_back(IntermediateRep("BEQ", compare_reg, "reg_0", my_labelA)); // branch to the else statement if condition evals to 0
        branchA->convertIR(dstreg, myContext, outStream);
        ...
        branchB->convertIR(dstreg, myContext, outStream);
